...
/**
 * CheckDieConstraints Class implements CheckConstraint
 * This class verifies if is possible to apply a card respecting die constraints
 * @author Luca Massini
 * @author Davide Lorenzi (JDocs)
 */

public class CheckDiePlacement implements CheckConstraint {

    /**
     * Move Verifier
     * This method calls the proper check rule to verify if is possible to apply a ToolCard or not. It
     * checks if all the placement rules would be respected in a placement in (newRow,newColumn) in the
     * player's grid
    ...
    private boolean isPossibleToMove(int newRow, int newColumn, Die chosenDie, Player player){
        ...
        return rules.checkAllRules(chosenDie,newRow,newColumn,playerGrid);
    }
